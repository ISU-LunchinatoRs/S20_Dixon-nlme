...
lfmc has three observations per plot.  These were destructively sampled, so they are independent replicates.  

Imagine instead that the same plants were sampled non-destructively.  If the data set has an ID variable, nested in plot, then you can connect the measurements on the same plant.  We want to fit models with two levels of random effects plot and ID within plot.  The nlme/lme syntax specifies both as plot/ID.

ID.csv is a made up data set where I have assumed that the first obs for each plot/time is ID 1, the second is ID 2, and the third is ID 3.
...
# Starting with the grouped data doesn't work: not run here
...
# this doesn't run because sb2.Grp has nested groups
#sb1.nlme <- nlme(lfmc ~ SSdlf(time, upper, lower, mid, scale), 
#  random = upper ~ 1,
#  data=sb2.Grp)

# you have to turn off the "nested groups" first
...
It would be great to use a parametric bootstrap to get confidence intervals and/or standard errors.  nlme includes a simulate.lme() function, but that fails when provided a non-linear fit.  Although nlme fits inherit the lme class, simulate.lme() seems to require an lme object.  Not run here
...
+    nlmer can have crossed random effects; nlme only allows nested.

#### Fitting the upper only RE model in nlmer
